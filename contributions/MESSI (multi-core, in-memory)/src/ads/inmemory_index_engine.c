...
void index_creation_pRecBuf_new(const char *ifilename, long int ts_num, isax_index *index)
{
    ...
    pthread_barrier_wait(&lock_barrier1);
     //#pragma omp parallel for num_threads(maxquerythread)
/*
    for ( i = 0; i < index->fbl->number_of_buffers; i++)
    {
        if(((parallel_fbl_soft_buffer*)(index->fbl->soft_buffers))[i].initialized)
        {
            for ( j = 0; j <maxquerythread ; j++)
                nodeid[i]=i;
                //parallel_fbl_soft_buffer current_buffer=((parallel_fbl_soft_buffer*)(index->fbl->soft_buffers))[i];
                nodesize[i]+=((parallel_fbl_soft_buffer*)(index->fbl->soft_buffers))[i].buffer_size[j];
        }
        else
        {
            nodeid[i]=i;
