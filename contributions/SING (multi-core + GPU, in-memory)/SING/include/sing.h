...
void isax_knn_binary_file_SING(const char *ifilename ,const char *labelfilename, int q_num,int k, isax_index *index,float minimum_distance, int min_checked_leaves,long int labelsize,pqueue_bsf (*search_function)(ts_type*, ts_type*, isax_index*, float, int,float*,float*,sax_type*,float*,float*,float*,node_list*,unsigned long int*,int));
