                 ...
                 int segments, int cardinality, int bit_cardinality)
{
    ...
    for (si=0; si<segments; si++) {
       ...
       //  Size of sub-array = cardinality - 1
        ...
        float *res = (float *) bsearch(&paa[si], &sax_breakpointsnew3[1], cardinality - 1,
                                    sizeof(ts_type), compare);
        if(res != NULL)
           sax_out[si] = (int) (res -  &sax_breakpointsnew3[1]);
        else if (paa[si] > 0)
            sax_out[si] = cardinality-1;
...
float   minidist_paa_to_isax_raw_SING_SIMD(float *paa, sax_type *sax, 
                           sax_type *sax_cardinalities,
                           sax_type max_bit_cardinality,
                           int max_cardinality,
                           int number_of_segments,
                           ...
                           float ratio_sqrt)
{
    ...
    int region_upper[16],region_lower[16];
    float distancef[16];
    int offset =0;// ((max_cardinality - 1) * (max_cardinality - 2)) / 2;
