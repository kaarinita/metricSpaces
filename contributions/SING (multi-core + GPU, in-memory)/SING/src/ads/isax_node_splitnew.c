...
int informed_split_decision (isax_node_split_data * split_data, 
    ...
    float * sax2paa = malloc(sizeof(float) * records_buffer_size * settings->paa_segments);
...
int informed_split_decision_nextbit (isax_node_split_data * split_data, 
                                     isax_index_settings * settings,
                                     isax_node_record * records_buffer,
                                     int records_buffer_size) {
    int segment_to_split = -1, 
        best_difference = records_buffer_size;

    int i, j;
    int current_splitting_bit, next_bit, current_difference;
    for (i = 0; i < settings->paa_segments; i ++) {
        current_splitting_bit = split_data->split_mask[i] + 1;

        if (current_splitting_bit >= settings->sax_bit_cardinality) {
            continue;
        } else {
            current_difference = 0;

            for (j = 0; j < records_buffer_size; ++ j) {
                next_bit = (records_buffer[0].sax[i] << current_splitting_bit) >> (settings->sax_bit_cardinality  - 1);
                current_difference += (next_bit == 1 ? 1 : -1);
            }

            if (abs(current_difference) < best_difference) {
                segment_to_split = i;
                best_difference = abs(current_difference);
            }
        }
    }
